...
module BuyBrowseApiClient
  ...
  class Aspect
    ...
    attr_accessor :localized_values
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::Aspect` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::Aspect`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'localized_values')
        if (value = attributes[:'localized_values']).is_a?(Array)
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          localized_values == o.localized_values
    end
    ...
    def hash
      [localized_name, localized_values].hash
    end
