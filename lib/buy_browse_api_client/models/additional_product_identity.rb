...
module BuyBrowseApiClient
  ...
  class AdditionalProductIdentity
    ...
    attr_accessor :product_identity
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::AdditionalProductIdentity` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::AdditionalProductIdentity`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'product_identity')
        if (value = attributes[:'product_identity']).is_a?(Array)
          self.product_identity = value
        end
      end
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          product_identity == o.product_identity
    end
