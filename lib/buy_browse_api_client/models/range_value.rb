  ...
  class RangeValue
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::RangeValue` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::RangeValue`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'exclusive_end')
        self.exclusive_end = attributes[:'exclusive_end']
      end

      if attributes.key?(:'exclusive_start')
        self.exclusive_start = attributes[:'exclusive_start']
      end
    ...
    def hash
      [_end, exclusive_end, exclusive_start, range, start].hash
    end
